...
def test_search(collection: chromadb.Collection) -> None:
    ...
    for query in test_queries:
        ...
        for i, (doc, meta) in enumerate(
            ...
            print(f"    마감월: {meta.get('deadline_month', '?')} | 스타트업: {meta.get('is_startup', '?')}")
            print(f"    최초저장일: {meta.get('first_saved_date', '?')}")
            print(f"    거리: {results['distances'][0][i]:.4f}")
            print(f"    문서: {doc[:80]}...")


def test_metadata_filter(collection: chromadb.Collection) -> None:
    """
    metadata 필터 검색을 테스트합니다.
    """
    print("\n=== metadata 필터 테스트 ===")

    print("\n[1] 마감월이 9월인 공고")
    september_results = collection.query(
        query_texts=["채용 공고"],
        n_results=5,
        where={"deadline_month": "09"}
    )

    if not september_results["metadatas"][0]:
        print("  결과 없음")
    else:
        for i, meta in enumerate(september_results["metadatas"][0]):
            print(
                f"  결과 {i + 1}: "
                f"{meta.get('company', '?')} | "
                f"{meta.get('title', '?')} | "
                f"마감월: {meta.get('deadline_month', '?')}"
            )
    ...
    if not startup_results["metadatas"][0]:
        print("  결과 없음")
    else:
        for i, meta in enumerate(startup_results["metadatas"][0]):
            print(
                f"  결과 {i + 1}: "
                f"{meta.get('company', '?')} | "
                f"{meta.get('title', '?')} | "
                f"스타트업: {meta.get('is_startup', '?')}"
            )
...
if __name__ == "__main__":
    ...
    test_metadata_filter(collection)
